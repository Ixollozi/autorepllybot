...
BRIEF_QUESTIONS = {
    ...
    DialogState.BRIEF_Q2: (
        "Что из этого у вас уже есть: логотип, фото работ/помещения, тексты услуг, "
        "Instagram/2GIS/сайт? Что есть — используем."
    ),
    DialogState.BRIEF_Q3: (
        "По наполнению: достаточно главная + услуги + контакты, "
        "или нужны ещё разделы — каталог / команда / акции?"
    ),
    DialogState.BRIEF_Q4: (
        "К какой дате хотели бы запуститься? "
        "(Ориентир для менеджера — в ТЗ дату сдачи не фиксируем.)"
    ),
}
...
def build_tz_summary(brief: dict) -> str:
    return (
        ...
        "Без коммерческих условий и сроков на этом шаге — "
...
PRICE_BRIDGE = (
    "Чтобы предложить варианты, сначала зафиксируем задачу — "
    "2–3 коротких вопроса, затем менеджер продолжит. "
